...
Stay alive as long as you can. The game gets harder over time. Avoid obstacles, don’t hit the ground or fly too high—and watch the scenery change every 25 seconds.
...
The game is in the **bmoney** repo under the `maze-game/` folder.
...
4. Save. After a minute or two, the site will be at:
   ...
   Your game will be at: **https://bmpalacio.github.io/bmoney/maze-game/**

To have the game at a **clean URL** (e.g. `username.github.io/stay-alive`):

- Create a **new repo** (e.g. `stay-alive`).
- Copy only `index.html` into the **root** of that repo (no `maze-game` folder).
- In that repo: **Settings → Pages → Deploy from branch → main → / (root)**.
- Your game will be at: **https://bmpalacio.github.io/stay-alive/**
...
3. Set **Base directory** to `maze-game` (or leave blank if you use a repo that has only the game).
4. **Publish directory** leave as `.` (or `maze-game` if base is the repo root).
5. Deploy. You’ll get a URL like `random-name.netlify.app`. In **Domain settings** you can add a custom domain.

### Option 3: Vercel (free)

1. Go to [vercel.com](https://vercel.com) and import your GitHub repo.
2. If the game is in `maze-game/`, set **Root Directory** to `maze-game`.
